...
import { PrijemnicaZaDobavljaca } from "../entity/PrijemnicaZaDobavljaca";
    ...
    try {
        if (req.query.kriterijum) {
            res.json(await createQueryBuilder("FakturaZaKupca", "faktura")
                ...
                .orWhere(`prijemnica.prijemnicaZaDobavljacaId like '%${req.query.kriterijum}%'`)
...
router.post("/", async (req: Request, res: Response) => {
    try {
        let pravilo = await getRepository(StavkaFaktureZaKupca).findOne(req.body.internoPraviloZaReklamaciju);
        if (!pravilo) {
